...
export const GlobalState = ({ children }) => {
  ...
  const airdrop = async () => {
    ...
    if (balance < LAMPORTS_PER_SOL) {
      ...
      await connection.TransactionConfirmationConfig(txHash);
    }
  };
  ...
  const createPost = useCallback(
    async (title, image) => {
      ...
      try {
        const postId = userAccount.lastPost.addn(1);
        const txHash = await program.methods
          .createPost(title, image)
          .accounts({
            postAccount: await getPostAccountPk(wallet.publicKey, postId.toNumber()),
            user: await getUserAccountPk(wallet.publicKey),
            owner: wallet.publicKey,
          })
          .rpc();
      ...
      try {
        const txHash = await program.methods
          ...
          .accounts({
            user: await getUserAccountPk(wallet?.publicKey),
            post: await getPostAccountPk(owner, id),
            matchAccount: await getLikeAccountPk(owner, id, liker),
            owner: wallet?.publicKey,
          })
          .rpc();
    ...
    async (owner, disliker, id) => {
      ...
      try {
        const txHash = await program.methods
          .unmatch()
          .accounts({
            like: await getLikeAccountPk(owner, disliker),
            post: await getPostAccountPk(owner, id),
            owner: wallet.publicKey,
          })
          .rpc();
